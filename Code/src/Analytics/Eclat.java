...
public class Eclat {
	...
	public ArrayList<ArrayList<Set<Integer>>> calculate(int min_sup) {
		...
		for(int item : m_db.getIDs()) {
			if( m_db.getClients(item).size() >= min_sup ) {
				...
				tmp.add(item);
				m_found_itemsets.get(itemset_size).add(tmp); //Add {item_id} to first set in m_found_itemsets
				m_itemsets.put(tmp, m_db.getClients(item)); //Save the pair { {item_id}, {clients that bought item with id=item_id} }
			}
		}
			...
			for( int idx1 = 0; idx1 < prev_iter_itemsets.size(); idx1++ ) {
				for( int idx2 = idx1 + 1; idx2 < prev_iter_itemsets.size(); idx2++ ) {
					...
					if(!itemset1.equals(itemset2) && compatible(itemset1, itemset2)) {
						ArrayList<Integer> new_tidlist = join(m_itemsets.get(itemset1), m_itemsets.get(itemset2)); //all the clients that bought itemset1 & itemset2
						Set<Integer> joined = new HashSet<Integer>(itemset1);
						joined.addAll(itemset2); //join itemset1 & itemset2
						if( new_tidlist.size() >= min_sup && !m_itemsets.containsKey(joined)) { //# of clients is above requirements and joined itemset doesn't exist already
	...
	private boolean compatible(Set<Integer> itemset1, Set<Integer> itemset2) { // 1 item verschilt tussen beide itemsets
